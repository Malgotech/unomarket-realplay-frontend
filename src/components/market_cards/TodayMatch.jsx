...
const TodayMatch = ({
  ...
  const handleTeam1ButtonClick = (e) => {
    ...
    if (!team1Colors || !team2Colors) {
      console.log("TodayMatch - Colors not loaded yet!");
      return;
    }

    console.log(
      "TodayMatch - handleTeam1ButtonClick - About to call onSelect with colors:",
      {
        team1Color: team1Colors.primary,
        team2Color: team2Colors.primary,
      }
  ...
  const handleDrawButtonClick = (e) => {
    e.stopPropagation(); // Prevent card click event
    console.log("TodayMatch - handleDrawButtonClick - colors:", {
      team1: team1Colors.primary,
      team2: team2Colors.primary,
    });
  ...
  const handleTeam2ButtonClick = (e) => {
    e.stopPropagation(); // Prevent card click event
    console.log("TodayMatch - handleTeam2ButtonClick - colors:", {
      team1: team1Colors.primary,
      team2: team2Colors.primary,
    });
    if (event.has_sub_markets) {
  ...
  const handleExpandClick = (e) => {
    ...
    console.log("TodayMatch - Expand icon clicked - Navigating to game view");
  ...
  if (isLoading) {
    return (
      <div className={`${isDarkMode ? 'bg-[#1A1B1E]' : 'bg-[#f7f7f7]'} rounded-[25px] p-5 flex flex-col justify-center items-center min-h-[300px] ${isDarkMode
  ...
  return (
    <div
      className={`${isDarkMode ? 'bg-[#1A1B1E]' : 'bg-[#f7f7f7]'} rounded-xl p-3 sm:p-5 flex flex-col relative cursor-default transition-all duration-200 ${isDarkMode
    ...
    >
      ...
      <div className="flex justify-between items-center z-10">
        ...
        <div className="flex items-center">
          ...
          {/* Only show expand button when hideExpandButton is false */}
          {!hideExpandButton && (
            <div
      ...
      <div className="hidden sm:flex justify-between items-center mt-4 sm:mt-8 flex-1 px-2 sm:px-8 z-10">
        {/* Left column - Team 1 */}
      ...
      <div className="sm:hidden flex flex-col mt-4 z-10 space-y-4">
        {/* Row 1: Team logos and names */}
        <div className="flex justify-between items-center px-4">
          ...
          <div className="flex flex-col items-center">
            {!allowOfficial ? (
              ...
              <img
                src={event?.team1_image || "https://via.placeholder.com/50"}
                alt={event?.team1_name}
                className="w-[50px] h-[50px] object-contain"
              />
            )}
            <div className="mt-2">
              <div className={`text-sm font-medium ${isDarkMode ? 'text-[#C5C5C5]' : 'text-zinc-800'} text-center`}>
                {allowOfficial ? event?.team1_name : getLastWord(event?.team1_name)}
              </div>
            </div>
          </div>
          ...
          {/* VS text */}
          <div className={`text-lg font-bold ${isDarkMode ? 'text-[#C5C5C5]' : 'text-zinc-800'}`}>
            VS
          </div>
          ...
          <div className="flex flex-col items-center">
            {!allowOfficial ? (
              ...
              <img
                src={event?.team2_image || "https://via.placeholder.com/50"}
                alt={event?.team2_name}
                className="w-[50px] h-[50px] object-contain"
              />
            )}
        ...
        {/* Row 2: Dots percentage volume - Show only when relevant prices are available */}
        {team1LastTrade !== null && team2LastTrade !== null && (
          <div className="flex flex-col items-center px-4">
            {/* Probability display with percentages */}
            <div className="flex items-center justify-center relative w-full">
              {/* Left percentage */}
              <div className={`${isDarkMode ? 'text-[#C5C5C5]' : 'text-[#2b2d2e]'} text-sm font-semibold`}>
                {team1LastTrade}%
              </div>

              {/* Probability circles - Smaller grid for mobile */}
              <div className="flex flex-col gap-[1px] w-[120px] h-[24px] mx-4">
                {[...Array(8)].map((_, rowIndex) => (
                  <div key={rowIndex} className="flex gap-[1px] w-full justify-center">
                    {[...Array(15)].map((_, colIndex) => {
                      const totalPosition = colIndex * 8 + rowIndex;
                      const dotPercentage = totalPosition * 0.83; // Adjust for smaller grid
                          ...
                          return (
                            <div
                              ...
                              style={{
                                ...
                                height: '3px',
                                backgroundColor: team1Colors.primary,
                                boxShadow: '0 0 4px ' + team1Colors.primary,
                              }}
                            />
                          );
                        } else if (dotPercentage < drawNormBoundary) {
                          ...
                          return (
                            <div
                              ...
                              style={{
                                ...
                                height: '3px',
                                backgroundColor: team2Colors.primary,
                                boxShadow: '0 0 4px ' + team2Colors.primary,
                              }}
                            />
                          );
                        }
                          ...
                          <div
                            ...
                            style={{
                              ...
                              backgroundColor: dotPercentage < (team1LastTrade || 50)
                                ? team1Colors.primary
                                : team2Colors.primary,
                              boxShadow: '0 0 4px ' + (dotPercentage < (team1LastTrade || 50)
                                ? team1Colors.primary
                                : team2Colors.primary),
                            }}
                          />
            ...
            {/* Volume display */}
            <div className="mt-2">
              <div className={`${isDarkMode ? 'text-[#C5C5C5]/50' : 'text-[#2b2d2e]/50'} text-xs font-medium`}>
                Vol ${event?.total_pool_in_usd?.toFixed(2) || "0.00"}
              </div>
            </div>
          </div>
        ...
        {/* Row 3: Buttons */}
        <div className="flex justify-center items-center px-4 space-x-2">
          {/* Team 1 button */}
          <div
            className="h-[36px] px-4 py-2 rounded-[5px] flex-1 flex justify-center items-center cursor-pointer transition-all duration-200 hover:brightness-90 active:brightness-85"
            style={{
              backgroundColor: team1Colors.primary,
            }}
          ...
          {/* Draw button - only show if has sub markets */}
      ...
      {/* Button row with responsive spacing - hidden on mobile */}
      <div className="hidden sm:flex justify-center items-center mt-4 mb-2 z-10 space-x-2 sm:space-x-10">
        {/* Yes button */}
        <div
