...
import { postData, fetchData } from "../../services/apiServices"; // Added fetchData
...
const Question = ({
  ...
  const handleMouseMove = (e) => {
    ...
    // We'll use this to shift the gradient breakpoint more toward the hovered side
    if (mousePosition < 50) {
      // Hovering on Yes side - make more green
      const greenIntensity = Math.max(0.5, 1 - mousePosition / 50); // Increases as mouse moves left
      ...
      const redIntensity = Math.max(0.5, (mousePosition - 50) / 50); // Increases as mouse moves right
      e.currentTarget.style.background = `linear-gradient(94deg, 
      ...
      e.currentTarget
        .querySelector(".yes-text")
        .classList.add("text-[#0d9488]");
  ...
  const handleBuy = async (e) => {
    ...
    try {
      ...
      const response = await postData(`api/user/orders/market/buy`, {
        event_id: res._id,
        market_id: activeMarket._id,
  ...
  // Handle card click to navigate to detail page with default selection (yes for first submarket)
  ...
  // Handle market name click to navigate to detail page with this specific market selected
  const handleMarketNameClick = (e, marketId) => {
    e.stopPropagation(); // Prevent the card click event from triggering

    // Get the previously selected option or default to 'yes'
    const market = res.sub_markets.find((m) => m._id === marketId);
    let selection = selectedMarkets[marketId];
    if (!selection) {
      selection = market.side_1;
    } else {
      selection = selection === "yes" ? market.side_1 : market.side_2;
    }
    // Navigate to detail page with the selected market and option
    navigate(
      `/market/details/${res._id}?marketId=${marketId}&selection=${selection}`
    );
  };

  // Effect to fetch order book data and set available shares
  useEffect(() => {
    if (showTradingPanel && activeMarket && selectedOption) {
      const fetchOrderBook = async () => {
        ...
        try {
          ...
          const data = await fetchData(
            `api/event/orderbook?market_id=${activeMarket._id}`
          );

          // Get dynamic side names for this market
          const side1 = activeMarket.side_1 || "Yes";
          const side2 = activeMarket.side_2 || "No";
          ...
          if (asks.length > 0) {
            ...
            console.warn(
              `No asks available for ${activeMarket.name} - ${selectedOption}`
            );
            setOrderBookAsks([]);
            setTotalAvailableShares(0);
          }
  ...
  }, [showTradingPanel, activeMarket, selectedOption]);

  // Effect to adjust purchaseQuantity when totalAvailableShares changes
  useEffect(() => {
    setPurchaseQuantity((currentQuantity) => {
      const newMax = totalAvailableShares > 0 ? totalAvailableShares : 0;
      return Math.min(currentQuantity, newMax);
    });
  ...
  useEffect(() => {
    ...
    if (quantityToFulfill > 0) {
      // This case implies purchaseQuantity > totalAvailableShares, which should be prevented by other logic.
      // If it happens, value is partial or indicates an issue.
      setApproxDollarValue("--"); // Or some indicator of partial fulfillment
      console.warn(
        "Could not fulfill entire purchase quantity from order book."
      );
    } else {
  ...
  // Read user's wallet balance and disable buy if cost exceeds wallet
  const walletBalance =
  ...
  return (
    <div>
      <div
        ...
        <div className="w-full flex justify-between gap-2 h-auto min-h-[60px] items-center">
          <img
            src={res.event_image || ""}
            alt="Event"
          ...
          <div className="w-[90%] relative group">
            <h1
              className={`text-[14px] font-semibold hover:underline hover:decoration-[2px] line-clamp-2 ${
              ...
              <React.Fragment key={market._id}>
                <div className="flex justify-between mb-3">
                  <div className="w-full mr-2">
                    {/* <h2
                      className={`text-[14px] transition-all duration-300 hover:cursor-pointer line-clamp-1 ${
                        isDarkMode
                          ? "text-[#C5C5C5] hover:text-[#C5C5C5]"
                          : "text-black hover:text-[#000]"
                      }`}
                      onClick={(e) => handleMarketNameClick(e, market._id)}>
                      {market.name}
                    </h2> */}
      ...
      ${isDarkMode ? "bg-[#2d2d2d] text-white" : "bg-black text-white"}
    `}>
                        {market.name}
                      </div>
                    </div>
                  ...
                  <div className="flex text-xl w-[48%] justify-end items-center  ">
                    ...
                    !(market.result && market.status === "settled") ? (
                      <h1
                    ...
                    {market.result && market.status === "settled" ? (
                      ...
                      <div
                        className={`w-fit  rounded flex justify-end gap-1 items-center h-[32px] transition-all duration-200    `}
                        // onMouseMove={handleMouseMove}
                        // onMouseLeave={handleMouseLeave}
                      >
                        <h1
                          className={`yes-text rounded-[6px] w-[50px] h-full flex justify-center items-center 
              bg-[#009443]
              ${
                selectedMarkets[market._id] === "yes"
                  ? "text-white font-bold"
                  : "text-white"
              }
              ${isDarkMode ? "text-[#A2FFF7]" : "text-[#0d9488]"}
              text-sm font-bold
              ...
              hover:cursor-pointer`}
                          onClick={(e) =>
                            handleSelection(e, market._id, "yes", market)
                          }>
                          {market.side_1}
                        </h1>

                        <h1
                          className={`no-text rounded-[6px] w-[50px] h-full flex justify-center items-center 
              bg-[#FF161A]
              ${
                selectedMarkets[market._id] === "no"
                  ? "text-[#fff] font-bold"
                  : "text-[#fff]"
              }
              ${isDarkMode ? "text-[#fff]" : "text-[#fff]"}
              text-[16px] font-bold
        ...
        <div className="flex justify-between mt-auto">
          ...
          <div className="flex">
            <div
              ...
              onClick={(e) => {
                e.stopPropagation();
                const fullUrl = `${window.location.origin}/market/details/${res._id}?selection=yes`;
                navigator.clipboard
                  .writeText(fullUrl)
                  .then(() => {
                    setShowCopied(true);
            ...
            {showCopied && (
              <div
                className={`absolute bottom-4 left-1/2 transform -translate-x-1/2 px-4 py-2 rounded flex items-center justify-center ${
                  isDarkMode
