...
import { extractColors, isDarkColor } from "../utils/colorExtractor";
import Loader from "./Loader";
import { useSelector } from "react-redux"; // Import useSelector for accessing theme state

const UpcomingMatch = ({ event, onSelect, onExpandClick, sportsSubcategories }) => {
  ...
  // Extract team colors from team images
  useEffect(() => {
    const loadTeamColors = async () => {
      try {
        setIsLoading(true);

        // Extract colors from team1 image
        if (event?.team1_image) {
          const colors1 = await extractColors(event.team1_image);
          setTeam1Colors(colors1);
        }

        // Extract colors from team2 image
        if (event?.team2_image) {
          const colors2 = await extractColors(event.team2_image);
          setTeam2Colors(colors2);
        }
    ...
    loadTeamColors();
  }, [event?.team1_image, event?.team2_image]);
  ...
  useEffect(() => {
    const checkIfLive = () => {
      // Use game_start_date if available, otherwise fall back to market dates
      const gameStartDate = event?.game_start_date || event?.match_start_date;
      
      if (gameStartDate) {
        const startTime = new Date(gameStartDate);
        const currentTime = new Date();

        // Match is live if it has started and has game_status data
        const hasStarted = currentTime >= startTime;
        const hasGameStatus = event?.game_status && (event.game_status.short || event.game_status.long);
        
        // For sports events, consider it live if:
        // 1. The game has started AND
        // 2. It has game status information (indicating it's actively being tracked)
        if (hasStarted && hasGameStatus) {
          setIsLive(true);
        } else {
          setIsLive(false);
        }
      ...
      >
        <div className="flex justify-between items-center">
          ...
          >
            ...
            >
              {formatMatchTime(event?.match_start_date)}
            </p>
          </div>
